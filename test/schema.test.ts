...
describe('Schema cleaning', () => {
    it('should keep enum values if below threshold and short string', () => {
        ...
        expect(cleaned.fields[0].enumValues).toEqual(["active", "inactive"]);
        expect(cleaned.fields[0].values).toBeUndefined(); // Should not leak original values array
    });
    ...
    it('should drop enum values if string is too long', () => {
        ...
        const cleaned = cleanSchema(rawSchema, 20);
        expect(cleaned.fields[0].enumValues).toEqual(["short"]);
        expect(cleaned.fields[0].values).toBeUndefined();
    });
    ...
    it('should drop enum values if above threshold', () => {
        ...
        expect(cleaned.fields[0].enumValues).toBeUndefined();
        expect(cleaned.fields[0].values).toBeUndefined();
    });
    ...
    it('should unconditionally drop values for non-string/number types (e.g. ObjectId, Date)', () => {
        ...
        expect(cleaned.fields[1].enumValues).toBeUndefined();
        expect(cleaned.fields[1].values).toBeUndefined();
    });
});
